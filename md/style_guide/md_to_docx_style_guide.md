...
Use $\widehat{x}$, not $\hat{x}$ (converts to widehat anyway).

Use $\overrightarrow{x}$, not $\vec{x}$ (converts to overrightarrow anyway).

Use $\widetilde{x}$, not $\tilde{x}$ (converts to widetilde anyway).
...
Use $\widehat{x}$, not $\hat{x}$ (converts to widehat on first trip, then stable).

Use $\overrightarrow{x}$, not $\vec{x}$ (converts to overrightarrow on first trip, then stable).

Use $\widetilde{x}$, not $\tilde{x}$ (converts to widetilde on first trip, then stable).

Use $a\, b$, not $a \: b$ (colon-space converts to backslash-space on first trip, then stable).

Use ${argmax}_{x}f(x)$, not $\operatorname{argmax}_{x} f(x)$ (loses operatorname wrapper on first trip, then stable).
